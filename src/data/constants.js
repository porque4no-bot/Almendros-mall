export const REND = {
  exc: 1.33, anillo: 0.377, fuste: 0.953,
  campana: {
...
export const EJES_Y = [{ label:"A", dist:0 },{ label:"B", dist:3.70 },{ label:"C", dist:11.11 },{ label:"D", dist:15.2 }];
export const EJES_X = [{ label:"1", dist:0 },{ label:"2", dist:8 },{ label:"3", dist:16 },{ label:"4", dist:24 },{ label:"5", dist:32 },{ label:"6", dist:41 },{ label:"7", dist:44.68 },{ label:"8", dist:48.66 }];
...
export const CAISSON_COORDS = (() => {
  ...
  for (const c of CAISSONS) {
    const axisKey = rev[c.k];
    if (!axisKey) { result[c.k] = { x:0, y:0, ex:"?", ey:"?" }; continue; }
    const [ey, ex] = axisKey.split("-");
    result[c.k] = { x: EJE_X_MAP[ex] ?? 0, y: EJE_Y_MAP[ey] ?? 0, ex, ey };
  }
  return result;
})();
